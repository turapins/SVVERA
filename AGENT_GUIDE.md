...
| **Ad creative & paid media** | `ad-creative` (copy variations, headlines, RSA), `ads` (campaign strategy, targeting, ROAS), `cro` (landing page conversion), `marketing-psychology` (persuasion, framing, anchoring) |
| **Content & copy** | `copywriting` (long/short-form), `copy-editing`, `emails` (sequences), `cold-email` (B2B outreach), `social` (organic social, platform-specific), `video` (video marketing strategy) |
| **Growth & acquisition** | `marketing-plan`, `marketing-ideas`, `lead-magnets`, `referrals`, `community-marketing`, `co-marketing`, `launch`, `product-marketing` |
| **SEO & discoverability** | `seo-audit`, `ai-seo`, `programmatic-seo`, `site-architecture`, `schema` |
| **Vercel / deployment** | `vercel-optimize`, `vercel-cli-with-tokens`, `deploy-to-vercel` |
